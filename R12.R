...
get_corr_snow_graph_Increment_CRYSTAL <- function(g_in,v_idx,b_in,da_in){
    g_out = g_in

    V(g_out)[v_idx]$m = V(g_out)[v_idx]$m + da_in 

    front = array(0,0)
    if(V(g_out)[v_idx]$m > 1) front = c(front,v_idx)
...
get_corr_snow_graph_Increment_TO_NONCLUSTER <- function(g_in,v_idx,b_in,da_in,d_topple_in){
    g_out = g_in
    V(g_out)[v_idx]$m = V(g_out)[v_idx]$m + da_in
    av_idxs = c()
    max_steps = length(V(g_out))
    i = 0
    while(max(V(g_out)$m)>1 && i<max_steps){
	g_out = get_corr_snow_graph_TOPPLE_TO_NONCLUSTER(g_out,which(V(g_out)$m>1),b_in,d_topple_in)
	i = i+1
    }
...
get_corr_snow_Memory_TO_NONCLUSTER = function(g_in_Global,N,a,b,d,da,d_topple){
	g = get_corr_snow_graph_Increment_TO_NONCLUSTER(g_in_Global - edges(g_in_Global)[[1]],sample(N,1),b,da,d_topple)
	return(list(graph = g, paths = shortest.paths(g)))
}

get_corr_snow_Memory_Fract = function(g_in_Global,N,a,b,d,da){
...
# q_borders=qb; ts = vals
my_bayes = function(q_borders, tss){
	hits = matrix(0,dim(q_borders)[1],dim(q_borders)[2]-1)
	for(i in 1:nrow(q_borders)) hits[i,] = diff(q_borders[i,]>tss[i])
	sd(rowSums(t(hits/length(tss))))	#sd((foreach(i=1:nrow(q_borders),.combine=sum)%do%)/length(tss))	
}

my_bayes_Cells = function(q_borders, tss){
	hits = matrix(0,dim(q_borders)[1],dim(q_borders)[2]-1)
	for(i in 1:nrow(q_borders)) hits[i,] = diff(q_borders[i,]>tss[i])
	rowSums(t(hits/length(tss)))
}
...
try_bayes2 = function(cf_tmp, vix){
	#print(cf_tmp)
	...
	if(min(foreach(i=1:P_CNT,.combine=c)%do%{ r=range(pms_tmp[,i]); MY_PARAMS_BND[[i]][1]<=r[1] && MY_PARAMS_BND[[i]][2]>=r[2] }) == 1) {
		...
		tmp_qb = #foreach(i=1:20,.combine=c)%do%{ 
			...
			foreach(i=1:TS_LEN)%do%{
				c(-Inf,
				as.numeric(quantile(MY_DISTRIBS[[which.min(as.numeric(rowSums(t(t(MY_PARAMS) - indic_to_param(INDICES[i,],cf_tmp))^2)))]]$data*vix[i], q_cnt_seq))[q_cnt_cnt],
				#as.numeric(quantile(MY_DISTRIBS[[which.min(as.numeric(rowSums(t(t(MY_PARAMS) - indic_to_param(INDICES[j*20+i,],cf_tmp))^2)))]]$data, q_cnt_seq))[q_cnt_cnt],
				Inf) }
